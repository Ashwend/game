...
Run headless:
  ...
  type: coal|iron|sulfur|stone   stage: 0|1|2
Geometry is deterministic per stage (identical silhouette across types, like the
procedural design); <type> only selects the COLOR_0 palette. Stone vein gets no
bright mineral chunks (its "chunks" are exposed darker rock), so it stays
visually distinct. Z-up authoring frame, bottom at z=0; export_yup -> game +Y up,
bottom at y=0 to match the procedural meshes' anchor. export_materials='NONE'
(~10 KB glbs); Rust builds the four shared StandardMaterials and supplies the
rock texture as base_color_texture.
...
def paint_face(f, col, uv, color_of):
    """Flat-shade a face: per-loop COLOR_0 = color_of(vert) and box UV."""
    n = f.normal
    for lp in f.loops:
        v = lp.vert
...
for k, (az, elev, r) in enumerate(st["chunks"]):
    ...
    # seat the nugget into the rock: pull toward the axis + drop it a touch so it
    # pokes out as an embedded lump rather than floating on the surface.
