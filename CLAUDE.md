...
| `building` | file + `building/` dir | Shared base-building domain rules, split by concern and re-exported flat: piece/tier taxonomy and dimensions in the root, `sockets.rs` (socket-snap geometry), `collision.rs` (multi-box colliders), `claims.rs` (Tool Cupboard claim cell math), `stability.rs` (support relations), `costs.rs` (cost/HP lookup over `game_balance`). Read by both client preview and server authority. |
| `cinematic` | file + `cinematic/` dir | Shared marketing-cinematic definitions: the pinned stage layout (`layout.rs`: seed, clear zones, authored node/base/prop/actor tables), the shot script (`script.rs`: shots, timings, camera keyframes), and camera path sampling (`camera.rs`). Server worldgen + orchestrator and the client director both read it; see docs/cinematic.md. |
...
| `control_socket` | file + dir | Dev/agent-only client control socket (`GAME_CONTROL_SOCKET`): serde wire shapes, Unix listener, and per-domain request handlers so automated tests can drive the client (screenshot, command, state dump). Unix + debug builds only; compiled out of releases. |
...
| `resource_nodes` | file | Resource-node definitions (`ResourceNodeDefinition`/`ResourceNodeModel`, in `RESOURCE_NODE_DEFINITIONS`) and gather rules. (`NodeKind` is the world/chunk pipeline's enum, not this module's.) |
...
| `server` | file + `server/` dir | Shared authoritative game state (`GameServer`) for loopback and dedicated alike. `server/` splits connection/auth, inventory, movement, dropped-item, resource-node, deployable, door, sleeping-bag, furnace (`furnace/state.rs`+`tick.rs`+`commands.rs`), and `chunk_manager/`. It also splits out `workbench.rs` (the tier-upgrade command via the generic upgrade table), `fuse.rs` (placed-charge fuse ticking), `explosion.rs` (`resolve_explosion` AoE), `defuse.rs` (claim-authorized defuse + refund), `heal.rs` (the server-clocked consumable use charge + the single `apply_player_heal` tail and its heal-over-time), `projectiles.rs` + `projectile_ecs.rs` (server ballistic sim + replicated projectile mirror), `meteor_shower.rs` (meteor scheduler/siting/impact), and `ruin_cache.rs` (world-spawned loot cache refill). The `HashMap`s on `GameServer` are authoritative; `*_ecs.rs` modules define the replicated mirror components. |
...
5. **Replicated state (six rules).** Every networked entity ships through Lightyear per-component replication, room-gated to the AoI chunk ring around each player. Authoritative `HashMap` on `GameServer` + ECS mirror entity, kept in sync by exclusive systems in `src/net/host/mirror.rs` (`sync_resource_node_entities`, `sync_dropped_item_entities`, `sync_deployable_entities`, `sync_projectile_entities`, `sync_player_entities`, `sync_loot_bag_entities`). The easy-to-break constraints:
   ...
   - **Client reconcilers are event-driven, not polling.** React to `Added<T>` and `RemovedComponents<T>`; do not iterate the full replicated query every frame (1-4 ms/frame noop cost at AoI scale ~1800 nodes). Canonical pattern in `src/app/systems/items/resource_nodes.rs` (pending-spawn `VecDeque`, reverse Entity->Id map, one-time catch-up scan after connect).
...
| Touch the cinematic stage map, `/cinematic` playback, or record the trailer | [docs/cinematic.md](docs/cinematic.md) |
