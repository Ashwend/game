...
- `auth`: WorkOS-backed identity. The server verifies a client's access-token JWT (RS256) against the WorkOS JWKS (`auth/verify.rs`) and derives the stable `u64` account id from the WorkOS subject claim (`auth/identity.rs`); the client owns PKCE login, token refresh, and OS-keychain storage under `auth/workos/`. `AuthMode` is `Workos` (default for dedicated) or `NoAuth` (loopback/localhost only).
