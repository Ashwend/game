...
| Tree bark + canopy + dead snags | (cel) | (cel) | (cel) | **Cel-shaded `ToonMaterial`, not `StandardMaterial`** (matte built in the shader). Trunk + solid faceted canopy each ride a per-species bark / foliage `ToonMaterial` (`texture * COLOR_0`, the ore path); dead snags reuse the bark material over a grey-`COLOR_0` glb. See "Trees" below + [Toon / cel shading](toon-shading.md). |
...
The low-poly props (sticks, stones, the tree LODs, impact debris; `src/app/scene/mesh/`) carry their colour in `Mesh::ATTRIBUTE_COLOR`, multiplied into a white `StandardMaterial`. **That attribute is linear RGBA**, it never goes through the sRGB decode `Color::srgb` performs, so a value eyeballed as a perceptual mid-grey (`0.55`) actually renders ~1.5-2x brighter (chalk white in daylight). This bit the prop set once already: every rock, trunk, and stick rendered pastel until the palette was rebuilt in linear terms. (The ore/vein nodes used to be in this list; they are now authored glbs, see below.)
...
- Identity needs to live in something readable at gameplay distance, not in tiny accents. The ore/vein nodes carry it **entirely in the studded mineral chunks** (boldly coloured: iron rust, coal near-black, sulfur yellow, stone-vein plain rock knobs); the rock body is one shared bright grey on every node. Keeping the rock grey is deliberate, an earlier "tint the whole mound toward the mineral" look read as a coloured blob; players expect to see *chunks of ore* in stone. Colours are baked as the glb `COLOR_0`, see `art/ore/build_ore.py`.
...
The **ore/vein nodes** are authored glbs (`art/ore/build_ore.py`): a large, smooth-shaded irregular boulder studded with a cluster of mineral chunks, one glb per type per depletion stage (4 types x 3 stages, `assets/ore/<type>/stage_<n>.glb`). The **rock body is one shared bright grey on every node** (`_ROCK` in the build script); the per-mineral identity lives entirely in the studded chunks' COLOR_0 (iron rust, coal near-black, sulfur yellow, stone-vein plain rock knobs).

Unlike every other prop, the ores are **cel-shaded**, not PBR. They use a standalone custom `OreToonMaterial` ([`src/app/scene/ore_toon.rs`](../src/app/scene/ore_toon.rs) + [`assets/shaders/ore_toon.wgsl`](../assets/shaders/ore_toon.wgsl)) that quantises the sun's half-Lambert into hard cel bands over `albedo = rock_texture * COLOR_0`, adds a flat ambient floor, and darkens the silhouette with a dark ink-style edge. One shared material covers all four ores (COLOR_0 differentiates). The texture is a soft hand-painted stylized stone (`assets/textures/ore/rock.png`, box-projected). This was a deliberate art-direction choice: the hand-painted look read flat under PBR, and the toon shader gives it the anime/cel look. Tuning lives in the material `params` (band count, ambient floor, ink-edge strength + width) so it can be tweaked without a shader recompile. Like terrain, it follows the standalone-Material / `@group(#{MATERIAL_BIND_GROUP})` Metal rules above; the hand-built cel path skips shadow *reception* (the ore still casts), the same trade the grass makes.
...
(the script header has the full ComfyUI-Flux + OpenCV pipeline:
[`art/comfy_gen.py`](../art/comfy_gen.py) references →
[`measure_tree.py`](../art/trees/measure_tree.py) silhouette →
[`make_tree_textures.py`](../art/trees/make_tree_textures.py) tiles →
[`build_trees.sh`](../art/trees/build_trees.sh) for all nine). Each glb is **two
meshes** loaded in [`assets.rs`](../src/app/scene/assets.rs): mesh 0 = the bark
**trunk**, mesh 1 = the **canopy**. The spawn path
...
mesh + material pair and the forest batches.

**Trees are part of the cel family.** Trunk + canopy ride the shared cel
[`ToonMaterial`](toon-shading.md) (PBR-then-posterise, `texture * COLOR_0`), not a
`StandardMaterial`. Five handles: pine/birch bark, pine/birch foliage, and a
dead-bark material (bark detail over the dead glb's grey `COLOR_0`). Conventions
that matter:

- **The canopy is SOLID faceted geometry, not alpha cards.** That is the whole
  reason trees can use the opaque `ToonMaterial` unchanged (no `discard`, no
  double-sided variant). Pine = a lumpy closed cone stack, birch = octa leaf
  blobs, both opaque. Because it is now **single-sided** (the cel material
  back-face culls, unlike the old `cull_mode: None` cards), `build_foliage` runs
  `recalc_face_normals` so every shell faces outward, otherwise the cull renders
  it inside-out.
- **Textures are decoded synchronously with a CPU mip chain** (`build_mip_chain`,
  shared with the terrain loader) + a repeat/aniso sampler (no mips for loaded
  PNGs in Bevy 0.18 → grain aliases into sparkle at range). The detail textures
  are soft, low-contrast painted cel grain (bark grooves / needle / leaf); the cel
  bands + ink edge carry the form, the texture only rides `COLOR_0`.
- **Up-biased canopy normals (`UP_BIAS = 0.5`).** The glb bakes per-vertex normals
  lerped halfway toward +Z (up, before the y-up export flip) so the canopy lights
  soft from the sky without dark-walling, while keeping real light/shadow FORM
  that the cel bands then quantise. Set in the Blender script, kept under the
  `recalc_face_normals` winding fix (which touches loop order, not the custom
  split normals keyed by vertex index).
- **Clumped canopy surfaces (`CONE_JITTER` / `BLOB_JITTER`).** Each cone ring and
  ...
  is a lumpy cluster with a ragged silhouette (which the cel ink edge then outlines)
  instead of a smooth lathe solid.
- **Vertex colours stay linear, textures load sRGB.** The glb COLOR_0 tones each
  canopy layer (darker lower → lighter crown, undersides darker) and the trunk's
  base-ring AO; the dead-snag trunk uses a cool-grey `COLOR_0` (`DEAD_GREY`) so the
  same bark detail reads weathered. Don't double-correct.
- **The trunk continues up through the canopy** as a thin tapering spine (to
  ~93% of the canopy top, `TRUNK_EXTEND_FRAC`), so it reads as a continuous trunk
  through the foliage gaps. The trunk tube is **capped at both ends** (a fan to a
  centre vertex, `recalc_face_normals` orienting the closed manifold) so a felled
  trunk shows a solid bark end, and each cone has a closed bottom cap so the
  canopy is a solid volume from below / during felling.
- **Felling fade.** Opaque cel props can't fade via `base_color` alpha, so
  `ToonMaterial` carries a per-instance `fade` uniform (binding 4). The felling
  effect clones the trunk + canopy materials and ramps `fade` 1→0; crossing below
  1.0 flips `ToonMaterial::alpha_mode` to `Blend` for the dissolve (see
  [`node_death.rs`](../src/app/systems/node_death.rs)). Static trees stay at
  `fade = 1.0` / opaque.
...
- **Dead snags are bark-trunk glbs too** (`assets/trees/dead_*`): a tapered bark
  trunk + bare branches, no canopy. They read weathered grey from the glb's
  cool-grey `COLOR_0` under `dead_bark_material` (the bark detail over grey), not a
  separate texture. A felled snag drops a bare trunk (the felling path checks
  `NetworkResourceNode.dead` and skips the canopy), not a sprouting live crown.
