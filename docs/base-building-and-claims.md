...
- **Combat lockout (repair AND upgrade).** A piece that took combat damage from any source (tool swing, projectile, blast) cannot be repaired or tier-upgraded for `BUILDING_REPAIR_COMBAT_LOCKOUT_TICKS` (5 minutes) after the last hit, so landed raid damage sticks instead of being patched from behind the wall mid-fight. Upgrades are covered because an upgrade refills HP to the new tier's full. The stamp lives in the transient `GameServer::recently_damaged` map (never persisted or replicated; a restart clears it); the three damage paths write it and `repair_lockout_remaining_ticks` reads it. Pinned by the two `*_locked_out_after_combat_damage_*` tests in `src/server/tests/building.rs`.
...
**Window shutter** (`DoorVariant::Shutter`, item `wood_shutter`, recipe `WOOD_SHUTTER_RECIPE_ID`: 3 `hewn_log` + 2 `plant_twine`, bench t1). A third door variant that mounts in a **window-wall** opening (`DoorVariant::mount_piece`) and is **codeless** (`DoorVariant::code_locked` = false): the set-code prompt is skipped on hang, and the E-toggle is gated on base authorization (owner, or authorized on the covering Tool Cupboard, via `building_modify_allowed`, the same gate hammer upgrades use), never on a code. Closed, its collider fills the window plane (`shutter_collider_blocks`), blocking movement, arrows, and targeting through the hole; open, the panel swings clear so defenders shoot out. WoodBuilding material at `SHUTTER_MAX_HP` (750), half the wood door: a shuttered window is a real but weaker soft spot. The stability graph supports a mounted panel from either opening kind (doorway or window wall); destroying the window wall destroys its shutter. Appending the variant bumped the save format to v22.
...
**HUD standing indicator.** While the player stands inside ANY claim footprint, a slim green/red bar sits directly above the health bar (bottom-right): green "Authorized" when a covering cupboard has them on its list, red "Unauthorized" otherwise; outside every claim it disappears entirely. The verdict is server-computed per tick (`claim_status_at` in `src/server/claim.rs`, the `building_modify_allowed` walk minus the outside-claim owner fallback) and ships on the owner-only `PlayerClaimStatus` component on the private player entity (`src/server/player_ecs.rs`); the widget is `src/app/ui/hud/claim.rs`. Server-side pin: `claim_status_reports_coverage_and_authorization_for_the_hud` in `src/server/tests/claim.rs`.

**Upkeep and decay** (`src/server/upkeep.rs`, constants under `UPKEEP_*` + `TOOL_CUPBOARD_SLOT_COUNT` in `src/game_balance.rs`). The cupboard carries a 12-slot storage grid (opened via the hold-E wheel's "Open Upkeep", riding the shared storage-box container flow with `ContainerViewKind::ToolCupboard`; the panel shows per-material rates, stock, days of cover, and a decay warning via `OpenLootBagView.upkeep`). Every `UPKEEP_PERIOD_TICKS` (5 real minutes, real-time like the meteor scheduler) each cupboard is billed per claimed building piece in the piece's tier material (sticks = wood, hewn = hewn logs, stone = stone) at `UPKEEP_PER_DAY_*` per in-game day (~10% of build cost daily); sub-integer period costs bank in the transient `CupboardState::upkeep_carry`. A material bucket the grid cannot cover in full pays nothing, flags `upkeep_unpaid`, and every claimed piece of that tier loses `UPKEEP_DECAY_PCT_PER_PERIOD` (5%) of max HP that period, destroyed at zero through the normal spill + stability path (a dry base rots to nothing in under two real hours). Restocking stops further decay at the next period; lost HP stays lost until repaired. Only building pieces are billed, only claimed pieces decay (unclaimed structures neither pay nor decay), and overlapping same-owner claims double-pay rather than a piece riding free. The grid persists via the generic `PersistedDeployedEntity.storage` (no format change); the carry/unpaid flags are transient. Pinned by `src/server/tests/upkeep.rs`.
...
| Satchel Charge (2,000, placed) | 100% | 85% | 45% | 30% |

Resulting point-blank raid math (tuned against the wall/door HP above): a hewn wood wall (3,600) falls to 5 kegs; a stone wall (6,000) to 7 satchels; an iron door (3,000) to exactly 5 satchels (600/charge), the only charge with a metal arm. The door ladder reads wood door 2 satchels, iron door 5, stone wall 7, so a door stays the designed breach point of any base. Revisit the metal column if a third door tier lands. The raid-economics tests in `src/server/tests/explosives.rs` pin these counts end to end through `resolve_explosion`.
