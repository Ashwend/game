...
- **Procedural Blender glb (a separate, parametric pipeline):** the full-detail
  live trees (pine + birch, three sizes) are authored glbs too, but generated by a
...
- **One mesh, one material slot, shared vertex material.** Unlike a tool (two
  ...
  the model's `COLOR_0` vertex colours tint it, exactly as the ore nodes and the
  tree glbs do. Do **not** call `glb_material` for these. Spawn is unchanged
