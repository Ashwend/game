...
- **Forest-fringe ore rule** (folded into `chunk_kind_target`): a `Forest` chunk holds no coal or sulfur (those stay in the high-risk barren biomes), gets a single lucky iron node where the raw ore channel `>= FOREST_IRON_ORE_CHANNEL (0.64)`, and a channel-scaled stone vein where the stone channel `>= FOREST_STONE_VEIN_CHANNEL (0.56)`. Qualifying forest chunks cluster on the edge of nearby barren biomes, so the strike reads as a lucky fringe find.
- **Stray-deposit rolls** (also in `chunk_kind_target`, hashed per chunk via `chunk_stray_hash` independently of the smooth channels): a forest or plains chunk has a `FOREST/PLAINS_STRAY_IRON_CHANCE_PCT` (10%) chance of a single iron node, and a forest chunk a `FOREST_STRAY_STONE_VEIN_CHANCE_PCT` (15%) chance of a single stone vein where the channel rule missed. This is what keeps large home-biome interiors from being ore deserts; coal and sulfur stay barren-only so the raid feedstock still forces trips into the dangerous biomes. The regrow capacity grid (`build_empty_grids`) passes the same hash, so generation and regrow ceilings agree.
...
### Ruin POIs: burnt-out houses (`src/world/ruins.rs`)

Ruins are burnt-out houses, homes gutted by the ongoing meteor storm, carrying restocking salvage chests: four hand-authored shell prefabs (`RuinPrefab::BurntCottage | BurntFarmhouse | BurntShed | BurntBarn`). Each prefab is ONE authored glb (`assets/ruins/<stem>.glb`, built by `art/ruins/build_ruins.py`: charred plank walls burnt to jagged stubs, a stone floor plinth, fallen rafters, rubble, a standing chimney on the cottage/farmhouse) plus a matching hand-authored `RuinBox` collider table (`plinth_boxes` + `wall_boxes`). A shell is a single mesh, so no two faces are coplanar by construction (the old building-piece kitbash z-fought where foundations abutted). Each prefab carries 1 to 3 cache points, every one sitting over a floor plinth (pinned by test, spawned at the plinth top, `FLOOR_TOP_M` = the building-foundation height so plinths walk like foundations).

`ruin_layout(world_seed, dims)` is the single shared scatter, a pure function of `(world_seed, dims)` exactly like the node pipeline; the layout is never persisted and is recomputed from the seed on every load. Placement is Poisson-style rejection sampling from a seeded `splitmix64` whole-world stream: `RUIN_SCATTER_CANDIDATES` (40) candidates are drawn across the playable interior and kept only if outside the centre exclusion ring (`RUIN_SPAWN_EXCLUSION_RADIUS_FRACTION` = 0.15 of the playable radius, keeping ruins away from the fresh-spawn area), inside `PlayableBounds` shrunk by `RUIN_BOUNDS_MARGIN_M` (12 m), and at least `RUIN_MIN_SPACING_M` (180 m) from every already-accepted site. All four constants live in `src/game_balance.rs`. A rejected candidate still draws its prefab and orientation, so the stream advances identically regardless of how many candidates pass. Sites orient in exact quarter turns (`RuinSite::quarter_turns`, yaw derived): the collision pipeline is AABB-only, and quarter turns keep every collider box exactly coincident with the authored shell instead of a loose enclosure that would block doorways.
...
- **`ruin_footprints`** (one circle per site, `footprint_radius_m` 4.5 to 8.5 m) gates node placement (`generate_chunk_spawns` rejects any resource-node candidate inside a footprint, and the regrow path passes the same footprints) AND player placement: the server rejects deployables, torches, and building pieces inside a footprint plus `RUIN_PLACEMENT_EXCLUSION_MARGIN_M`, with the client ghost mirroring the gate (explosive charges exempt).
- **`RuinSite::static_blocks`** registers each site's collision/LoS geometry as `WorldBlock`s in `WorldData::chunk_world` (the perimeter-wall precedent, so the `BlockGrid` gives collision, projectile LoS, and melee LoS for free). The boxes come from the prefab's `RuinBox` tables (mirrored by hand in the build script; edit the two together), tagged `BlockKind::RuinMasonry` so the client scene skips the plain cuboid and draws the authored shell.
- **`RuinSite::cache_points`** drive the owner-less `RuinCache` salvage chests, RECONCILED on every load (`GameServer::new` in `src/server/lifecycle.rs`): persisted chests standing on a live cache point are kept, chests stranded off-point (an old save whose layout has since changed; they'd otherwise sit buried in the reworked shells as invisible colliders) are dropped, and any missing point spawns a stocked chest. A brand-new world is just the all-points-missing case. Chest restock and loot rules live in [docs/crafting-and-deployables.md](crafting-and-deployables.md).
- **The site transform** (`x`, `z`, `RuinSite::yaw()`) feeds the client's shell mesh spawns (`src/app/scene/world.rs`, two primitives per shell: charred timber on the toon wood material, masonry on the toon stone material), computed client-side from the same seed. (Ruins deliberately do NOT appear on the world map: the burnt-house glyphs were removed alongside the meteor marker so ruins are discovered by exploring; `crate::world::ruins` is still consumed by worldgen and the meteor obstacle avoidance.)
...
`SAVE_FORMAT_VERSION = 21` (`src/save/format.rs`). A version mismatch is **rejected**, never migrated (`decode_world_save` bails on `version != SAVE_FORMAT_VERSION`), and the worlds-screen surfaces it as a "couldn't load" banner. The full v1..v21 changelog is documented inline above the constant. Recent bumps: v19 added the `DeployableKind::RuinCache` variant plus `PersistedDeployedEntity::ruin_cache` (the salvage chest restock schedule); v20 added the `DeployableKind::Explosive { kind }` variant (placed blackpowder charges) plus `PersistedDeployedEntity::fuse`; v21 retired the ancient item vocabulary (persisted ids `meteorite`/`ancient_fittings` became `meteorite_alloy`/`salvaged_fittings`, plus the new `meteorite_ingot`) with no struct drift, and is rejected anyway because a v20 save would otherwise load full of stacks whose ids no longer resolve.
