...
- **The site transform** (`x`, `z`, `RuinSite::yaw()`) feeds the client's shell mesh spawns (`src/app/scene/world.rs`, two primitives per shell: charred timber on the toon wood material, masonry on the toon stone material) and the world-map burnt-house glyphs (`src/app/ui/world_map.rs`), both computed client-side from the same seed.
