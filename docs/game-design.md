...
The Hammer does 0% to everything (`ToolKind::Hammer, _ => 0`): it builds, it never raids. This closes the "hammer as a free raid tool" hole outright. Explosives are now the answer to stone: the effectiveness matrix runs a powder bomb at 40% of wood and 0% of metal up to a satchel at 85% of wood and 45% of stone, so a stone-wall breach costs 7 satchels. Metal only takes the satchel's token 8% until the retired ember charge's top-tier replacement lands. The raid-cost tuning is pinned by `src/server/tests/explosives.rs`.
...
The day/night cycle is authoritative and shared (`src/world_time.rs` - `WorldTime`). A full 24 h in-game day spans 30 real minutes at the default multiplier (`REAL_SECONDS_PER_DAY = 30 * 60`). Torches are placeable 8-hour light sources (`TORCH_BURN_TICKS`, 8 game-hours). There is a hold-M world map with biome legend, per-player markers, permanent ruin landmark glyphs, and a temporary meteor shower impact marker during an event. The meteor shower is the one world event that punctuates the cycle; otherwise the clock is ambience and orientation and does not gate the loop.
