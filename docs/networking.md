...
- World events: `MeteorShower { meteors: Vec<MeteorStrike> }` (the meteor-event announce; each `MeteorStrike` is `{ impact_position, impact_tick, trajectory_seed, size }`, 4 to 5 per scheduled shower, one for `/meteor-here`). Broadcast once at T minus the warning window and resent (with all still-live meteors) to any client that connects while the event is alive (through the last post-impact crater window) via the connection/welcome path. The client computes the whole sky show, per-meteor danger warning, and craters as a deterministic function of this one payload plus its authoritative-clock estimate (`crate::world::meteor_shower`); there is deliberately no countdown or map-marker chrome, and a meteor is never per-tick replicated. Follows the `WorldTime` precedent: an event via a message, not a replicated entity, because the trajectory is a function of time.
