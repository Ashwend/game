...
sources:
  ...
  - src/app/ui/hud/claim.rs - claim_indicator
  ...
  - src/app/scene/meteor_sky.rs - MeteorVisual, MeteorRig, MeteorTrailSegment
...
### Claim standing indicator (bottom-right, above health)

`claim_indicator` (`src/app/ui/hud/claim.rs - claim_indicator`) is a slim status strip stacked directly above the health bar in the shared `hud_bars` area: green "Authorized" while the local player stands inside a Tool Cupboard claim they are authorized on, red "Unauthorized" inside someone else's, and absent entirely outside every claim (the open world draws no extra chrome). It reads the server-computed `PlayerClaimStatus` off `LocalPlayerState.private`, never a client-side guess; the server half is documented in [base-building-and-claims.md](base-building-and-claims.md).
...
### Meteor shower: per-meteor warning and sky/ground VFX

The meteor shower event is announced by `ServerMessage::MeteorShower { meteors }` (4 to 5 size-varied strikes with staggered impact ticks) and stored on `runtime.meteor_showers`; all of its chrome is computed client-side from that payload plus the authoritative clock estimate, so it costs nothing on the wire and cannot desync. **The event is deliberately unannounced**: no countdown pill, no chat line, and no world-map marker (the permanent ruin map glyphs were removed in the same pass; the map shows only biomes, the grid, and the player's own pins). The sky fireballs and the audio are the announcement.

- **HUD.** `meteor_shower_hud` (`src/app/ui/hud/meteor_shower.rs - meteor_shower_hud`) draws ONLY the escalating "Evacuate the area" warning, and only while the player's own position is inside `METEOR_SHOWER_DANGER_RADIUS_M * size` of a live, not-yet-landed meteor; every meteor of the shower is evaluated and the most imminent covering one drives the escalation (`meteor_shower_danger_intensity` ramps it over the final 60 s). It goes quiet per meteor after that meteor's impact, when its crater visual takes over.
- **Scene VFX.** The sky pre-spawns `METEOR_SKY_RIGS` = 5 fireball rigs, each a `MeteorVisual` body with a `MeteorTrailSegment` streak and `MeteorFlameTongue` licks, every entity tagged `MeteorRig` (`src/app/scene/meteor_sky.rs`); meteor `k` of the shower drives rig `k`, evaluated against the FRACTIONAL clock estimate (`ClientRuntime::server_tick_precise`): truncating to whole 20 Hz ticks quantises the plunge into 50 ms steps that stutter at render frame rates. A meteor's `size` multiplies the rendered ball radius (and thins the ember/airburst counts), so the whole presentation scales per meteor. Each fireball keeps the tuned polish: opaque vertex-gradient flame tongues, a thin additive HDR glow rim past the shell that feeds bloom, far-phase shimmer amplification (brightness only, never mesh scale), proxy drama growth converging back to the true angular size over the final descent, and a one-time per-rig mid-flight airburst `METEOR_AIRBURST_VISIBLE_FRACTION` of the way through the flight THIS client has watched (a core flash plus `MeteorFragment` mini-meteors that sputter embers and burn up in the sky; pure presentation, fired only on a watched crossing so a stale pop never replays, and a short `/meteor-here` dive still gets its burst partway in). At each impact `update_meteor_shower_ground_system` (`src/app/scene/meteor_shower.rs - update_meteor_shower_ground_system`) spawns a `MeteorShowerCrater` rig keyed by that meteor's `impact_tick`, uniformly scaled by its `size` (valid because the shared profile is a homothety of size): the dug-in crater as two meshes sharing a seam ring (`build_crater_mesh`: a SOLID opaque bowl+rim body plus a translucent vertex-alpha burn skirt fading out over the grass), with geometry sampled from the shared `crate::world::crater_surface_height` profile that also drives the movement collider's analytic multi-crater floor (players walk over the mounds, see docs/movement.md) and the server's crater-node seating, plus scattered `MeteorShowerSiteFire` particle-fire emitters (furnace-style flame puffs + embers with flickering lights, count/heat scaled by size, driven per meteor by `animate_meteor_shower_site_fire_system`) that burn for `METEOR_SHOWER_SITE_FIRE_SECONDS`, fade out, and despawn, leaving the crater for the rest of the window. The one-time rock blast at each strike is fixed-size matte `ImpactChip` physics debris on seeded `irregular_rock_mesh` variants (count/speed/flash scaled by size); its tumble bleeds off with ground friction and bounces so settled chunks lie still, and everything self-despawns. The module also owns the strike cues, per meteor: the impact boom is pre-armed `IMPACT_BOOM_LEAD_S` early so the file's baked lead-in ends on that impact frame (gain scaled by size via `meteor_size_gain_db`), the flyby bed keys to the LOUDEST (nearest, size-weighted) in-flight meteor (`meteor_rumble_driver`) and starts at a fixed `FLYBY_LEAD_S` before that meteor's strike so its silent tail lands on it (fade-out, never a mid-waveform cut; it re-keys to a still-crossing sibling after the driver lands), and each strike fires a distance-scaled `CameraImpactKick::trigger_meteor_impact` (a small meteor kicks like a farther one). Entirely client-side and derived from the event state with no replicated crater entity or save bump. A late joiner who connects during a crater phase gets the resent announce (all still-live meteors) and draws the sites too (fires only while each burn window is open, never the blast replay).
