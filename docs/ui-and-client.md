...
sources:
  ...
  - src/app/ui/hud.rs - meteor_shower_hud, ranged_hud, ranged_hud_view
  ...
  - src/app/scene/sky.rs - MeteorVisual, MeteorTrail
...
- Crafting tab: `src/app/ui/crafting/` (`mod.rs`, `recipes.rs`, `filter.rs`, `list.rs`, `details.rs`, `icon.rs`), a master/detail browser: the left column is the searchable, category-filtered recipe list (real item icons, a craftable-status dot per row, click to select), the right column is a detail card for the selected recipe (description, per-ingredient have/need lines, batch quantity stepper, Craft button). `CraftingUiState::selected_recipe` stores the selection; a hidden or unset id falls back to the top visible entry (`effective_selection_index` in `mod.rs`), so the card is never empty. The Max button fills the quantity field with the largest affordable batch rather than enqueueing instantly.
