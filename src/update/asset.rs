...
//!
//! Names mirror the `asset:` entries in `.github/workflows/release.yml`. Each
//! constant is gated to exactly one host so a build only ever knows about the
//! archive that carries *its* binary; on any host we don't publish a build for,
//! the name is empty and self-update degrades to "open the download page".
