//! Server-authority tests for the workbench open/close commands and the
//! tier-upgrade path (range gate, affordability, table-row lookup, and the
//! tier-2 station unlock).
...
use crate::{
    ...
    items::{
        DeployableKind, IRON_BAR_ID, METEORITE_INGOT_ID, SALVAGED_FITTINGS_ID, WORKBENCH_T1_ID,
    },
    protocol::{DeployedEntityId, ItemStack, ServerMessage, ToastKind, Vec3Net, WorkbenchCommand},
...
fn open_missing_entity_warns() {
    ...
    let out = server.apply_workbench_command(
        client,
