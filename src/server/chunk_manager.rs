...
use crate::game_balance::{
    RESOURCE_REGROW_MAX_TICKS as MAX_REGROW_TICKS, RESOURCE_REGROW_MIN_TICKS as MIN_REGROW_TICKS,
};
...
impl ChunkManager {
    ...
    pub fn from_save(save: ChunkManagerSave, now_tick: u64) -> Self {
        ...
        Self {
            ...
            loot_bag_chunks: HashMap::new(),
            regrow_queue,
            next_node_id: save.next_node_id.0.max(1),
    ...
    pub fn to_save(&self, now_tick: u64) -> ChunkManagerSave {
        ...
        ChunkManagerSave {
            ...
            dims: self.dims.dims,
            next_node_id: crate::protocol::ResourceNodeId(self.next_node_id),
            node_chunks,
            pending_regrows,
        }
    }
...
/// here, and the cap derivation defers to
/// [`kind_target`](crate::world::chunk::kind_target), the same
