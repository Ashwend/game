...
fn tree_is_dead_for_vitality(forest_channel: f32, id: ResourceNodeId) -> bool {
    ...
    let r = (splitmix64(id.0) >> 40) as f32 / (1u64 << 24) as f32;
    r >= alive_chance
}
...
mod tests {
    ...
    fn tree_vitality_dead_in_open_alive_in_forest() {
        ...
        for id in 0..64u64 {
            assert!(
                !tree_is_dead_for_vitality(0.9, crate::protocol::ResourceNodeId(id)),
                "id {id} alive in forest"
            );
            assert!(
                tree_is_dead_for_vitality(0.2, crate::protocol::ResourceNodeId(id)),
        ...
        let dead: Vec<bool> = (0..256u64)
            .map(|id| tree_is_dead_for_vitality(mid, crate::protocol::ResourceNodeId(id)))
            .collect();
        ...
        for id in 0..256u64 {
            assert_eq!(
                dead[id as usize],
                tree_is_dead_for_vitality(mid, crate::protocol::ResourceNodeId(id))
            );
        }
    }
}
