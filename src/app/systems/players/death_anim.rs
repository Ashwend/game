//! Remote-player death collapse and the logged-out sleeping pose: the
//! `DyingPlayer` / `SleepingPlayer` state components, the kick/fall/bounce/fade
//! tuning constants, and the tick system that plays the collapse out.

use bevy::prelude::*;

use crate::{app::PLAYER_VISUAL_CENTER_Y, protocol::ClientId};
...
pub(crate) struct DyingPlayer {
    ...
    pub(super) roll_axis: Vec3,
    pub(super) roll_magnitude: f32,
...
/// limp face-plant.)
pub(super) fn lying_transform(upright: Transform) -> Transform {
...
pub(super) fn compute_fall_axes(client_id: ClientId, current: Transform) -> (Vec3, Vec3, f32) {
    let seed = client_id.0.wrapping_mul(0x9E3779B97F4A7C15);
...
mod tests {
    ...
    fn compute_fall_axes_yields_horizontal_unit_axes() {
        let (fall_axis, roll_axis, roll) =
            compute_fall_axes(crate::protocol::ClientId(42), Transform::IDENTITY);
    ...
    fn compute_fall_axes_is_deterministic_per_client_id() {
        ...
        let a = compute_fall_axes(crate::protocol::ClientId(7), Transform::IDENTITY);
        let b = compute_fall_axes(crate::protocol::ClientId(7), Transform::IDENTITY);
        assert_eq!(a.0, b.0);
        assert_eq!(a.1, b.1);
        assert_eq!(a.2, b.2);

        // Different ids (very likely) produce a different roll magnitude.
        let other = compute_fall_axes(crate::protocol::ClientId(8), Transform::IDENTITY);
