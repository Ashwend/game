//! Day/night visuals.
//!
//! The procedural [`Atmosphere`](bevy::light::Atmosphere) on the camera (set up
...
/// Camera pose read by the sky systems. Shared with `meteor_sky`, whose
/// fireball placement is camera-relative.
pub(super) type CameraTransformQuery<'w, 's> =
...
/// Clamped lerp, shared with `meteor_sky` (its descent ramps use the same curve).
