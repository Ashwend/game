...
- Default image engine: RESEARCHED 2026-07-19 (web pass). Recommendation: upgrade the Flash worker to **Qwen-Image-2512** (Feb 2026, 7B, Apache-2.0, strongest open T2I in blind evals, LoRA-training-stable, ControlNet Union available, sibling **Qwen-Edit-2511** for iterative refinement) as the quality engine, keep flux-schnell as fallback; **Z-Image-Turbo** (6B, Apache-2.0, ~8-step fast) as the cheap/fast tier. Anime-specialized SDXL checkpoints (Animagine XL 4 / Illustrious / NoobAI) have the strongest native anime look but murky per-checkpoint licenses (Fair-AI-PL lineage, Civitai forks); read the specific license before ever shipping their output. The BIGGER win is workflow: a style LoRA trained on our accepted icon set + ControlNet canny/scribble silhouette control (both supported by Qwen-2512) attacks the actual dud modes (disassembled tools, crossed compositions, style drift) that no base-model swap fixes. Verify exact HF repo ids + licenses at implementation per the license-discipline rule.
...
- D3: ~~Keep Claude-driven measured Blender authoring as the primary mesh path for held items.~~ **Superseded 2026-07-19 (owner decision):** held items go through the RunPod image-to-3D lane too, same as the ore family: generated reference -> TRELLIS.2 native low-poly export (~10k tris, its own baked texture kept as-is) -> canonicalize + socket + lean export. The anime/cel red-line applies to every item. Measured Blender authoring remains the fallback if the pilot shows the lane cannot hold a thin haft.
...
**Redefined 2026-07-19 (owner decision), EXECUTED same day via `art/held/`:** the pilot is the five gathering tools (`wood_stone_hatchet`, `wood_stone_pickaxe`, `iron_hatchet`, `iron_pickaxe`, `iron_sickle`) through the image-to-3D family pipeline (art/ore template): 4 icon + 4 mesh-reference candidates each through the 2D lane, owner picks in pick.html, then TRELLIS.2's NATIVE low-poly export (`--decimation-target ~10000`; final decision after two preview.html A/B rounds: the ore-style 1.5k flat recipe faceted the tools, the improved 6k smooth local recipe still eroded thin features via the voxel remesh, and the worker's own simplifier keeps full silhouette fidelity with its own baked texture), `socket_grip` authored into each built glb (Phase 0 convention below), and socket-driven attach in `held.rs` replacing the per-item offset stack for these five. **Status: all five generated, built, wired, and verified in-game (first-person, mid-swing, remote rig, paperdoll, icons); old `.blend`s + sickle scripts + `art/concepts/sickle/` retired; docs updated.** Same-day round 2 (owner direction): the split icon/reference generation was replaced by a UNIFIED set (one picked image = both icon and mesh reference; separate generations made the icon and the model look like different items), the whole set restyled toonier, pickaxes pinned to the classic curved silhouette, the sickle re-grounded against real reference anatomy, and the build gained PCA canonicalization + the metallic-zeroed bake (the true root cause of every "black steel" bake, superseding round 1's floor-lift-as-symptom). Phase 0 items now real: the socket convention (item 1), the socket-driven attach for all three views (item 2, legacy constants remain for not-yet-rebuilt items). Still open from Phase 0: the export validator (item 3), golden-screenshot acceptance (item 4), palette/trim assets (item 5, superseded in spirit by the per-item baked-albedo approach the owner chose). The bow's functional sockets wait for the follow-up batch. Learned this pilot: TRELLIS meshes a reference's painted ground shadow as real geometry (`trim_ground_disc` knob), and its dark-steel bakes can land ~0.001 linear, needing the `ALBEDO_CURVE` floor lift into the 0.05-0.09 viewmodel band. Original pilot list kept for the record:
...
Acceptance per item: validator passes, headless screenshots look right in all four views with the per-item override table EMPTY, icon silhouette match, swing feel unchanged or better. Only after all pass with empty overrides do we proceed; if overrides were needed, fix the convention, not the items.
